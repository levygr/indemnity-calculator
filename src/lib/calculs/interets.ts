...
export interface LigneInterets {
  ...
  anatocisme: boolean;
  dateAnatocisme: string | null;
  // Régime "decision_5pts"
  dateDecision: string | null;
  dateExecutoire: string | null;
  delaiMajorationMois: number; // défaut 2
  // Régime "badinter_apres"
...
export const LIBELLES_REGIME: Record<RegimeInterets, string> = {
  taux_legal: "Taux légal simple (avant jugement)",
  badinter_avant: "Doublement Badinter avant jugement (art. L. 211-13 C. assur.)",
  decision_5pts: "Intérêts sur décision, majoration + 5 points (art. L. 313-3 CMF)",
  badinter_apres: "Majoration Badinter après décision (art. L. 211-17 C. assur.)",
};
...
export function phasesPourLigne(ligne: LigneInterets): PhaseTaux[] | null {
  ...
  switch (ligne.regime) {
    ...
    case "decision_5pts": {
      if (!ligne.dateExecutoire) return null;
      const bascule = ajouterMois(ligne.dateExecutoire, ligne.delaiMajorationMois);
      return [
        { aPartirDe: debut, multiplicateur: 1, majorationPoints: 0 },
        { aPartirDe: bascule, multiplicateur: 1, majorationPoints: 5 },
      ];
    }
    case "badinter_apres": {
      ...
      return [
        { aPartirDe: debut, multiplicateur: 1, majorationPoints: 0 },
        { aPartirDe: p1, multiplicateur: 1.5, majorationPoints: 0 },
        { aPartirDe: p2, multiplicateur: 2, majorationPoints: 0 },
      ];
    }
  }
}
