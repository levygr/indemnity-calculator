...
function formulaLibelle(taux: number, mult: number, eff: number): string {
...
function Page() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      ...
      <Alert>
        <AlertTitle>Intérêts avant jugement</AlertTitle>
        <AlertDescription className="text-sm space-y-1">
          <p><strong>Taux légal simple</strong> : intérêts au taux légal sur les périodes fixées par la décision, capitalisables sur demande (art. 1343-2 C. civ.).</p>
          <p><strong>Doublement Badinter</strong> : taux légal doublé au titre de la sanction prévue par l'article L. 211-13 du Code des assurances, applicable avant jugement.</p>
        </AlertDescription>
      </Alert>
                ...
                <TableBody>
                  {resultat.value.segments.map((s, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell className="text-xs">{formulaLibelle(s.tauxLegalBase, s.multiplicateur, s.tauxAnnuel)}</TableCell>
